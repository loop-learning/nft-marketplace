...
const ListNFTPage = () => {
  ...
  const { createListing, useReadApproval, createAuction, approveNFT, isPending, isConfirmed, isConfirming, error } = useMarketplace();

  const { data, isFetched, error: approvalError } = useReadApproval(nftData.nftContract, nftData.tokenId)
            ...
            <div className="bg-gray-800 rounded-lg p-6">
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    ...
                    className={`p-4 rounded-lg border-2 transition-colors text-left ${listingType === "fixed"
                  ...
                  <button
                    ...
                    className={`p-4 rounded-lg border-2 transition-colors text-left ${listingType === "auction"
                      ? "border-purple-500 bg-purple-900/20"
                      : "border-gray-600 bg-gray-700"
                      }`}
                  >
                    <div className="flex items-center space-x-3 mb-2">
                      <Gavel size={20} className={listingType === "auction" ? "text-purple-400" : "text-gray-400"} />
                      <span className={`font-medium ${listingType === "auction" ? "text-purple-300" : "text-gray-300"}`}>
                        Auction
                      </span>
                    </div>
                    <p className={`text-sm ${listingType === "auction" ? "text-purple-200" : "text-gray-400"}`}>
              ...
              {isConfirmed && showNotification && (
                <div className="mb-6 p-4 bg-green-600 rounded-lg">
                  <div className="text-white text-sm">
                    {didApprove ? "NFT Approved" : listingType === "fixed" ? "Listing" : "Auction"} created successfully!
                    <button
                      type="button"
                      onClick={resetForm}
              ...
              {error && (
                <div className="mb-6 p-4 bg-red-600 rounded-lg">
                  <div className="text-white text-sm">
                    <strong>Error:</strong>{handleError(error)}
                  </div>
                </div>
              )}
              ...
              <div className="space-y-6">
                ...
                {listingType === "auction" &&
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNftData({ ...nftData, duration: e.target.value })}
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-lg font-semibold transition-colors disabled:bg-gray-600 ${listingType === "fixed"
                ...
                >
                  ...
                  ) : isConfirmed && showNotification ? didApprove ? "Approved" :
                    `${listingType === "fixed" ? "Listing" : "Auction"} Created!`
                    : (
                      !isApproved ? "Approve" : `Create ${listingType === "fixed" ? "Listing" : "Auction"}`
                    )}
                </button>
              </div>
            </div>
