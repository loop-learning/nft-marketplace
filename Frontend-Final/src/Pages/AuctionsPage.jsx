...
const AuctionsPage = () => {
  ...
  const { useActiveAuctions, fetchMetadata, metadataCache, formatPrice } = useMarketplace();

  const { data: listings, isLoading: loadingAuctions, error } = useActiveAuctions(0, 100)

  const [dummyNFTs] = useState(generateDummyAuctions());
  ...
  const blockchainNFTs = useMemo(() => {
    ...
    return listings.map((listing) => {
      ...
      return {
        ...
        isFixed: false,
        endTime: Number(listing.endTime.toString())
      }
    })
  }, [listings, metadataCache])

  const allAuctions = useDummyData ? dummyNFTs : blockchainNFTs;
  ...
  const filteredAuctions = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case "price-low":
          return parseFloat(a.currentBid || a.price) - parseFloat(b.currentBid || b.price);
        case "price-high":
          return parseFloat(b.currentBid || b.price) - parseFloat(a.currentBid || a.price);
        case "bid-count":
  ...
  const totalVolume = allAuctions.reduce((sum, auction) => sum + parseFloat(auction.currentBid || auction.price), 0);
  const activeAuctions = allAuctions.filter(auction => auction.isActive !== false).length;
    ...
    <div className="min-h-screen bg-gray-900">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="bg-gray-800 rounded-lg p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-white text-lg font-semibold">{activeAuctions}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-800 rounded-lg p-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-white text-lg font-semibold">{endingSoon}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mb-4 flex justify-between items-center">
          <p className="text-gray-400">{filteredAuctions.length} active auctions</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
          {paginatedAuctions.map((auction) => (
            <div key={auction.id} className="relative group">
              <div className="bg-gray-800 rounded-lg overflow-hidden hover:shadow-xl transition-shadow cursor-pointer"
                ...
                <div className="p-4">
                  ...
                  <p className="text-gray-400 text-sm truncate">{auction.collection}</p>
                  <div className="mt-2">
                    <div className="flex justify-between items-center">
                      <span className="text-gray-400 text-xs">Current Bid</span>
                      <span className="text-green-400 font-bold">
                        {auction.currentBid !== 0n ? formatPrice(auction.currentBid) : formatPrice(auction.startingPrice)} ETH
                      </span>
                    </div>
                    {auction.endTime && (
              ...
              {auction.endTime && formatTimeRemaining(auction.endTime) !== "Ended" && (
                <div className="absolute top-2 right-2">
                  {(() => {
                    const timeLeft = new Date(auction.endTime) - new Date();
                    const isUrgent = timeLeft < 24 * 60 * 60 * 1000; // Less than 24 hours
                    return (
                      <div className={`text-white text-xs px-2 py-1 rounded ${isUrgent ? 'bg-red-600 animate-pulse' : 'bg-gray-800/80'
                        }`}>
                        {formatTimeRemaining(auction.endTime)}
                      </div>
                    );
