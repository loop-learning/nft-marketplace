...
contract NFTMarketplace is Ownable, ReentrancyGuard {
    ...
    event MarketplacePause();
    event MarketplaceUnpause();
    ...
    function acceptOffer(uint256 offerId) external nonReentrant whenNotPaused {
        ...
        // Check if NFT is currently in an active auction and end it
        uint256 auctionId = nftToAuction[offer.nftContract][offer.tokenId];
        if (auctionId != 0 && auctions[auctionId].active) {
            Auction storage auction = auctions[auctionId];
            
            // Refund current bidder if there is one
            if (auction.currentBidder != address(0)) {
                payable(auction.currentBidder).transfer(auction.currentBid);
            }
            ...
            // End the auction
            auction.active = false;
            nftToAuction[offer.nftContract][offer.tokenId] = 0;
        ...
        nftToOffer[offer.nftContract][offer.tokenId] = new uint256[](0);
        nftToListing[offer.nftContract][offer.tokenId] = 0;
        ...
        returns (Auction[] memory)
    {
        ...
        Auction[] memory results = new Auction[](returnCount);
        uint256 currentIndex = 0;
        uint256 resultIndex = 0;
        ...
        for (
            ...
            if (auctions[i].active) {
                if (currentIndex >= offset) {
                    results[resultIndex] = auctions[i];
                    resultIndex++;
                }
                currentIndex++;
            }
    ...
    function getOfferOfUser(address user)
        external
        view
        returns (Offer[] memory)
    {
