...
import { X, ShoppingCart, Gavel, TrendingUp, Tag } from "lucide-react";
import { useAccount } from "wagmi";
import { useMarketplace } from "../hooks/useMarketplace";
import { useEffect } from "react";


const NFTModal = ({ nft, isOpen, setIsOpen, onClose }) => {
  ...
  const { purchaseListing, cancelListing, makeOffer, acceptOffer, placeBid, endAuction, isExpired, formatPrice, useOffersForNFT, isConfirmed, isConfirming, isPending, error } = useMarketplace()

  const { data: offers } = useOffersForNFT(nft?.contractAddress, nft?.tokenId)
  ...
  const handleMakeOffer = async () => {
    ...
    try {
      await makeOffer(nft.contractAddress, nft.tokenId, offerAmount, offerDuration);
  ...
  const isOwner = address && nft.seller && address.toLowerCase() === nft.seller.toLowerCase();
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-700 rounded-lg p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {nft.traits && nft.traits.length > 0 ? nft.traits.map((trait, index) => (
                  <div
                    key={index}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-700 rounded-lg p-4">
              ...
              {isConfirmed && (
                <div className="mb-4 p-3 bg-green-600 rounded-lg">
                  <div className="text-white text-sm">Transaction confirmed!</div>
                </div>
              )}
              ...
              {error && (
                <div className="mb-4 p-3 bg-red-600 rounded-lg">
                  <div className="text-white text-sm">Error: {error.message}</div>
                </div>
              )}
              ...
              {!isConnected ? (
                <div className="text-center py-4">
                  <p className="text-gray-400 mb-2">Connect wallet to interact with this NFT</p>
                </div>
              ) : isOwner ? (
                ...
                <div className="space-y-2">
                  ...
                  {!nft.isFixed && nft.endTime && formatTimeRemaining(nft.endTime) === "Ended" && (
                    <button
              ...
              {showOfferForm && (
                <div className="mt-4 p-4 bg-gray-600 rounded-lg">
                  <div className="mt-4 p-4 bg-gray-600 rounded-lg">
                    <h4 className="text-white font-semibold mb-3">Make an Offer</h4>
                    <div className="space-y-3">
                      <div>
                        <label className="block text-gray-300 text-sm mb-1">Offer Amount (ETH)</label>
                        <input
                          type="number"
                          step="0.001"
                          value={offerAmount}
                      ...
                      <div>
                        <label className="block text-gray-300 text-sm mb-1">Offer Duration (days)</label>
                        <select
                          value={offerDuration}
              ...
              {showBidForm && (
                <div className="mt-4 p-4 bg-gray-600 rounded-lg">
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="block text-gray-300 text-sm mb-1">Bid Amount (ETH)</label>
                      <input
                        type="number"
                        step="0.001"
                        value={bidAmount}
                        ...
                        placeholder="0.00"
                        min={parseFloat(nft.currentBid || nft.startingPrice || nft.price) + 0.001}
                      />
                      <div className="text-xs text-gray-400 mt-1">
                        Minimum bid: {(parseFloat(nft.currentBid || nft.startingPrice || nft.price) + 0.001).toFixed(3)} ETH
                      </div>
                    </div>
                    <div className="flex space-x-2">
                      <button
                        onClick={handlePlaceBid}
                        disabled={!bidAmount || parseFloat(bidAmount) <= parseFloat(nft.currentBid || nft.startingPrice || nft.price) || isPending || isConfirming}
                        ...
                        <div key={index} className="bg-gray-700 rounded-lg p-3 flex justify-between items-center">
                          <div>
                            ...
                            <div className="text-gray-400 text-xs">
                              Expires: {new Date(Number(offer.expiration) * 1000).toLocaleDateString()}
                            </div>
                          </div>
                          <button
                            onClick={() => handleAcceptOffer(offer.offerId || index)}
                            disabled={isPending || isConfirming}
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-white font-medium">{nft.collection || 'Unknown'}</span>
              </div>
            </div>
          </div>
        </div>
